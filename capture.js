...
const videoState = {
  ...
  broadcast: new BroadcastChannel('channel-0'),
  broadcast1: new BroadcastChannel('channel-1'),
  reading: true,
  ID: null
...
/** RESOURCE - https://developer.mozilla.org/en-US/docs/Web/API/Broadcast_Channel_API */
  videoState.broadcast1.onmessage = (event) => {
if(event.data)    
try{
   console.log(event.data.result)
   clearInterval(videoState.ID)
   videoState.reading = false
   copyToClipboard(event.data.result)
   const notificationToast = () => {ons.notification.toast(`${event.data.result}`, {timeout: 1000})}
   takePicture(notificationToast) 
}
catch(e)
  {
...
const processImg = () => {
  ...
  if(videoState.reading = true) {
    ...
    if (width && height) {
      ...
      //console.log('Text' + JSON.stringify(imgData1))
      videoState.broadcast.postMessage({imgData,width,height})
...
const clearPhoto = () => {
  ...
  if (videoState.ID) 
    {
    clearInterval(videoState.ID);
    videoState.ID = setInterval(processImg, 250);
    }
...
const initiateStream = async (opts) => {
  try {
    ...
    /** setInterval() **/
    clearInterval(videoState.ID);
    videoState.ID = setInterval(processImg, 1000);
...
const startup = () => {
  ...
  elements.startButton.addEventListener('click', (e) => {
    e.preventDefault();
    //takePicture(processImg);
    //setInterval(videoState.ID,250)
    //clearInterval(processImg)
  });
