...
//self.importScripts('lib/alignpat.js')
...
const broadcast = new BroadcastChannel('channel-0')

  broadcast.onmessage = event => {
    
    qrcode.width = event.data.width
    qrcode.height = event.data.height
    qrcode.imagedata= event.data.imgData
  ...
  try
  {
   ...
   // console.log(result)
    ...
    const broadcast1 = new BroadcastChannel('channel-1')
    broadcast1.postMessage({result})
  }
    catch(e)
      {
